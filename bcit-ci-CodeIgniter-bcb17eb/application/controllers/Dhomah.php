...
class Dhomah extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Dhomah - Huruf Hijaiyah';
        $data['huruf_list'] = $this->get_dhomah_huruf();
        $this->load->view('dhomah_view', $data);
    }

    public function detail($huruf) {
        $data['huruf'] = $huruf;
        $data['huruf_data'] = $this->get_huruf_detail($huruf);
        $data['title'] = 'Detail Dhomah - ' . $huruf;
        $this->load->view('detail_dhomah_view', $data);
    }

    public function quiz() {
        $data['title'] = 'Quiz Dhomah';
        $data['questions'] = $this->generate_quiz_questions();
        $this->load->view('dhomah_quiz_view', $data);
    }

    public function practice() {
        $data['title'] = 'Latihan Dhomah';
        $data['practice_huruf'] = $this->get_random_huruf();
        $this->load->view('dhomah_practice_view', $data);
    }

    private function get_dhomah_huruf() {
        return array(
    ...
    private function get_huruf_detail($huruf) {
        $huruf_list = $this->get_dhomah_huruf();
    ...
    private function generate_quiz_questions() {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $questions[] = array(
                'question' => 'Bagaimana cara membaca huruf ' . $random_huruf['arabic'] . ' dengan dhomah?',
                'huruf_arabic' => $random_huruf['arabic'],
                'correct_answer' => $random_huruf['latin'] . 'u',
                'options' => array(
