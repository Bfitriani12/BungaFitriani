...
class Tanwindhomah extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Tanwin Dhomah - Huruf Hijaiyah';
        $data['huruf_list'] = $this->get_tanwindhomah_huruf();
        $this->load->view('tanwindhomah_view', $data);
    }

    public function detail($huruf) {
        $data['huruf'] = $huruf;
        $data['huruf_data'] = $this->get_huruf_detail($huruf);
        $data['title'] = 'Detail Tanwin Dhomah - ' . $huruf;
        $this->load->view('detail_tanwindhomah_view', $data);
    }

    public function quiz() {
        $data['title'] = 'Quiz Tanwin Dhomah';
        $data['questions'] = $this->generate_quiz_questions();
        $this->load->view('tanwindhomah_quiz_view', $data);
    }

    public function practice() {
        $data['title'] = 'Latihan Tanwin Dhomah';
        $data['current_huruf'] = $this->get_random_huruf();
        $this->load->view('tanwindhomah_practice_view', $data);
    }

    private function get_tanwindhomah_huruf() {
        return array(
    ...
    private function get_huruf_detail($huruf) {
        $huruf_list = $this->get_tanwindhomah_huruf();
    ...
    private function generate_quiz_questions() {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $questions[] = array(
                'question' => 'Bagaimana cara membaca huruf ' . $random_huruf['arabic'] . ' dengan tanwin dhomah?',
                'huruf_arabic' => $random_huruf['arabic'],
                'correct_answer' => $random_huruf['latin'] . 'un',
                'options' => array(
                    $random_huruf['latin'] . 'un',
                    $random_huruf['latin'] . 'a',
                    $random_huruf['latin'] . 'in',
                    $random_huruf['latin']
                )
            );
        }
