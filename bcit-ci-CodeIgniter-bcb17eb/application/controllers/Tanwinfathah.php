...
class Tanwinfathah extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Tanwin Fathah - Huruf Hijaiyah';
        $data['huruf_list'] = $this->get_tanwinfathah_huruf();
        $this->load->view('tanwinfathah_view', $data);
    }

    public function detail($huruf) {
        $data['huruf'] = $huruf;
        $data['huruf_data'] = $this->get_huruf_detail($huruf);
        $data['title'] = 'Detail Tanwin Fathah - ' . $huruf;
        $this->load->view('detail_tanwinfathah_view', $data);
    }

    public function quiz() {
        $data['title'] = 'Quiz Tanwin Fathah';
        $data['questions'] = $this->generate_quiz_questions();
        $this->load->view('tanwinfathah_quiz_view', $data);
    }

    public function practice() {
        $data['title'] = 'Latihan Tanwin Fathah';
        $data['current_huruf'] = $this->get_random_huruf();
        $this->load->view('tanwinfathah_practice_view', $data);
    }

    private function get_tanwinfathah_huruf() {
        return array(
    ...
    private function get_huruf_detail($huruf) {
        $huruf_list = $this->get_tanwinfathah_huruf();
    ...
    private function generate_quiz_questions() {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $questions[] = array(
                'question' => 'Bagaimana cara membaca huruf ' . $random_huruf['arabic'] . ' dengan tanwin fathah?',
                'huruf_arabic' => $random_huruf['arabic'],
                'correct_answer' => $random_huruf['latin'] . 'an',
                'options' => array(
                    $random_huruf['latin'] . 'an',
