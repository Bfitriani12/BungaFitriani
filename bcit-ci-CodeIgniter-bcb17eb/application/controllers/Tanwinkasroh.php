...
class Tanwinkasroh extends CI_Controller {
    ...
    public function index() {
        $data['title'] = 'Tanwin Kasroh - Huruf Hijaiyah';
        $data['huruf_list'] = $this->get_tanwinkasroh_huruf();
        $this->load->view('tanwinkasroh_view', $data);
    }

    public function detail($huruf) {
        $data['huruf'] = $huruf;
        $data['huruf_data'] = $this->get_huruf_detail($huruf);
        $data['title'] = 'Detail Tanwin Kasroh - ' . $huruf;
        $this->load->view('detail_tanwinkasroh_view', $data);
    }

    public function quiz() {
        $data['title'] = 'Quiz Tanwin Kasroh';
        $data['questions'] = $this->generate_quiz_questions();
        $this->load->view('tanwinkasroh_quiz_view', $data);
    }

    public function practice() {
        $data['title'] = 'Latihan Tanwin Kasroh';
        $data['current_huruf'] = $this->get_random_huruf();
        $this->load->view('tanwinkasroh_practice_view', $data);
    }

    private function get_tanwinkasroh_huruf() {
        return array(
    ...
    private function get_huruf_detail($huruf) {
        $huruf_list = $this->get_tanwinkasroh_huruf();
    ...
    private function generate_quiz_questions() {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $questions[] = array(
                'question' => 'Bagaimana cara membaca huruf ' . $random_huruf['arabic'] . ' dengan tanwin kasroh?',
                'huruf_arabic' => $random_huruf['arabic'],
                'correct_answer' => $random_huruf['latin'] . 'in',
                'options' => array(
